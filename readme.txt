...
[KomentBox](http://nlpcaptcha.in/ "KomentBox") is a commenting service and tool for web comments and discussions.

KomentBox makes commenting easier with many features, up to nth level reply.

== Installation ==

To install in regular WordPress and [WordPress MultiSite](http://codex.wordpress.org/Create_A_Network):

1. Upload the `wp-komentbox` folder to the `/wp-content/plugins/` directory
1. Activate the plugin through the `Plugins` menu in WordPress
1. Get the NLPCaptcha API keys [here](http://nlpcaptcha.in "NLPCaptcha API keys") 
...
There are four common issues that NLPCaptcha caters to:

1. **Moderation Emails**: NLPCaptcha marks comments with a wrongly entered captcha as spam. This way, all the bots, which are the usual kinds of spam, are filtered out. But even for the other comments you have an option of switching on the moderation emails. This way even those comments which have been written after validating captcha will be posted on your website page only after you approve them. This saves any spam from going onto your website.
...
1. **Human Spammers**: Believe it or not, there are people who are paid (or maybe slave labor?) to solve CAPTCHAs all over the internet and spam. This is the last and rarest reason for which it might appear that NLPCaptcha is not working, but we have a solution for that as well. A combination of NLPCaptcha and Akismet might help to solve this problem, and if spam still gets through, there are multiple options of blocking users, creating blacklisted words and lastly manual moderation of comments. These all take care of any minute chances of human spammers.
